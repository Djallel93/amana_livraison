...
function validateLivraison(livraison) {
  ...
  if (livraison.latitude === undefined || livraison.latitude === null) {
    ...
    typeof livraison.latitude !== "number" ||
    livraison.latitude < -90 ||
    livraison.latitude > 90
  ) {
  ...
  if (livraison.longitude === undefined || livraison.longitude === null) {
    ...
    typeof livraison.longitude !== "number" ||
    livraison.longitude < -180 ||
    livraison.longitude > 180
  ) {
  ...
  if (
    livraison.nombre_personnes === undefined ||
    livraison.nombre_personnes === null
  ) {
    result.addError("nombre_personnes est obligatoire", "nombre_personnes");
  } else if (
    typeof livraison.nombre_personnes !== "number" ||
    livraison.nombre_personnes < 1
  ) {
  ...
  if (livraison.priorite === undefined || livraison.priorite === null) {
    ...
    typeof livraison.priorite !== "number" ||
    livraison.priorite < 1 ||
    livraison.priorite > 5
  ) {
...
function validateRoute(route) {
  ...
  // Validation distance
  if (
    route.distance_totale_km !== undefined &&
    route.distance_totale_km !== null
  ) {
    if (
      typeof route.distance_totale_km !== "number" ||
      route.distance_totale_km < 0
    ) {
      result.addError(
        "distance_totale_km doit être >= 0",
        "distance_totale_km",
      );
    }
...
function validateEtape(etape) {
  ...
  if (etape.ordre_passage === undefined || etape.ordre_passage === null) {
    ...
    typeof etape.ordre_passage !== "number" ||
    etape.ordre_passage < 1
  ) {
  ...
  for (const [field, rules] of Object.entries(schema)) {
    ...
    if (rules.type === "string") {
      ...
      if (rules.maxLength !== undefined && value.length > rules.maxLength) {
        result.addError(
          `${field} doit contenir au maximum ${rules.maxLength} caractères`,
          field,
        );
      }
    }
...
function checkDuplicateIds(ids) {
  ...
  if (duplicates.length > 0) {
    result.addError(`IDs en double détectés: ${duplicates.join(", ")}`, "ids");
  }
