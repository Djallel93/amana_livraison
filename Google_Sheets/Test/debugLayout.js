...
function makePhantomBlob() {
  return Utilities.newBlob(
    Utilities.base64Decode(PHANTOM_B64),
    "image/png",
    "phantom.png",
  );
}
...
function clearParasites(cell) {
  while (cell.getNumChildren() > 0) {
    ...
    if (
      ch.getType() === DocumentApp.ElementType.PARAGRAPH &&
      ch.asParagraph().getText() === ""
    ) {
      ch.removeFromParent();
    } else {
      break;
...
function testLabelLayout() {
  ...
  var qrBlobs = labels.map(function (lb) {
    return fetchQr(lb.url, dims.qrPx);
  });
  Logger.log("QR OK");

  body
    .appendParagraph("RECTO")
    .setFontSize(8)
  ...
  body
    .appendParagraph("VERSO")
    .setFontSize(8)
    .setForegroundColor("#7EA6D0")
    .setSpacingBefore(0)
    .setSpacingAfter(4);
  ...
  doc.saveAndClose();
  Logger.log(
    "OK : https://docs.google.com/document/d/" + doc.getId() + "/edit",
  );
}
...
function fillRectoCell(outerCell, label, dims, phantomBlob) {
  ...
  rc.setPaddingTop(sp)
    .setPaddingBottom(sp)
    .setPaddingLeft(sp)
    .setPaddingRight(sp + 2);
  var pR = rc.appendParagraph(label ? label.part : "");
  pR.setAlignment(DocumentApp.HorizontalAlignment.CENTER)
    .setSpacingBefore(0)
    .setSpacingAfter(0)
    .setLineSpacing(100);
  pR.editAsText()
    .setFontSize(dims.fontPartPt)
    .setBold(false)
    .setForegroundColor("#FFFFFF");
}
...
function buildFakeLabels(n) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    result.push({
      ...
      famille: "F_" + (10000 + i),
      part: part + "/" + total,
      url: "https://example.com/confirm?id=" + i,
    });
  }
  return result;
}
