...
function testBatchResolveLocation() {
  ...
  for (const key in result) {
    ...
    if (location) {
      ...
      Logger.log(
        `    Secteur: ${location.secteur ? location.secteur.nom : "N/A"}`,
      );
      Logger.log(
        `    Quartier: ${location.quartier ? location.quartier.nom : "N/A"}`,
      );
    } else {
      Logger.log(`  ${key}: NULL`);
    }
...
function testCompleteWorkflow() {
  ...
  Logger.log(
    "Coordonnées: " + JSON.stringify(geocodeResult[testFamily.adresse]),
  );
  ...
  if (geocodeResult[testFamily.adresse]) {
    ...
    Logger.log(
      "Localisation: " + JSON.stringify(resolveResult[coordKey], null, 2),
    );
  }
}
...
function testGeoApiDirect() {
  ...
  try {
    ...
    const geocodePayload = {
      action: "batchgeocode",
      adresses: [{ adresse: "10 Rue de Rivoli, Paris" }],
    };
    ...
    const resolvePayload = {
      ...
      coordinates: [{ lat: 47.2154056, lng: -1.6112647 }],
    };
    const resolveResponse = callGeoApi(
      "batchresolvelocation",
      resolvePayload,
      "POST",
    );
...
function showGeoApiConfig() {
  ...
  Logger.log(
    "API URL: " + (baseUrl ? baseUrl.substring(0, 50) + "..." : "NON DÉFINIE"),
  );
  Logger.log(
    "API KEY: " + (apiKey ? apiKey.substring(0, 10) + "..." : "NON DÉFINIE"),
  );
}
