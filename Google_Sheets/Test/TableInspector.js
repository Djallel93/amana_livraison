...
function inspectTable(table, tableIdx) {
    ...
    Logger.log('');
    Logger.log('┌─────────────────────────────────────────────────');
    Logger.log('│ TABLEAU #' + tableIdx + '  (' + numRows + ' lignes × ' + numCols + ' colonnes)');
    Logger.log('│ Border width : ' + safeGet(function () { return table.getBorderWidth(); }));
    Logger.log('│ Border color : ' + safeGet(function () { return table.getBorderColor(); }));
    Logger.log('└─────────────────────────────────────────────────');
    ...
    if (numRows > 3) {
        Logger.log('');
        Logger.log('  ... (' + (numRows - 3) + ' ligne(s) intermédiaire(s) non affichées)');
    }
}
...
function inspectCell(cell, r, c, indent) {
    ...
    Logger.log(indent + '  │  background   : ' + safeGet(function () { return cell.getBackgroundColor(); }));
    Logger.log(indent + '  │  paddingTop    : ' + safeGet(function () { return cell.getPaddingTop(); }) + 'pt');
    Logger.log(indent + '  │  paddingBottom : ' + safeGet(function () { return cell.getPaddingBottom(); }) + 'pt');
    Logger.log(indent + '  │  paddingLeft   : ' + safeGet(function () { return cell.getPaddingLeft(); }) + 'pt');
    Logger.log(indent + '  │  paddingRight  : ' + safeGet(function () { return cell.getPaddingRight(); }) + 'pt');
    Logger.log(indent + '  │  verticalAlign : ' + safeGet(function () { return cell.getVerticalAlignment(); }));
    Logger.log(indent + '  │  width         : ' + safeGet(function () { return cell.getWidth(); }) + 'pt');
    ...
    for (var i = 0; i < cell.getNumChildren(); i++) {
        ...
        if (type === DocumentApp.ElementType.PARAGRAPH) {
            ...
            Logger.log(indent + '  │  [' + i + '] Tableau imbriqué → inspection récursive');
...
function inspectParagraph(para, idx, indent) {
    ...
    Logger.log(indent + '  │       align        : ' + safeGet(function () { return para.getAlignment(); }));
    Logger.log(indent + '  │       spacingBefore : ' + safeGet(function () { return para.getSpacingBefore(); }) + 'pt');
    Logger.log(indent + '  │       spacingAfter  : ' + safeGet(function () { return para.getSpacingAfter(); }) + 'pt');
    Logger.log(indent + '  │       lineSpacing   : ' + safeGet(function () { return para.getLineSpacing(); }));
    Logger.log(indent + '  │       fontSize      : ' + safeGet(function () { return para.editAsText().getFontSize(0); }) + 'pt');
    Logger.log(indent + '  │       bold          : ' + safeGet(function () { return para.editAsText().isBold(0); }));
    Logger.log(indent + '  │       color         : ' + safeGet(function () { return para.editAsText().getForegroundColor(0); }));
    ...
    for (var i = 0; i < para.getNumChildren(); i++) {
        ...
        if (child.getType() === DocumentApp.ElementType.INLINE_IMAGE) {
            ...
            Logger.log(indent + '  │       [img] width=' + img.getWidth() + 'px  height=' + img.getHeight() + 'px');
        }
    }
}
