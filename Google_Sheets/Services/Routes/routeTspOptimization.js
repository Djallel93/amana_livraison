...
function optimizeDeliveryOrder(deliveries, hqCoords) {
  ...
  Logger.log(
    `[TSP] 📏 Original: ${Math.round(originalDistance)}km → Optimized: ${Math.round(optimizedDistance)}km (${improvement}% better)`,
  );
...
function nearestNeighborTSP(deliveries, hqCoords) {
  ...
  for (let i = 0; i < deliveries.length; i++) {
    ...
    if (i < 5 || i === deliveries.length - 1) {
      Logger.log(
        `[TSP]   ${i + 1}. ${deliveries[nearestIndex].id_livraison} (${Math.round(shortestDistance * 1000)}m from previous)`,
      );
    } else if (i === 5) {
    ...
    for (let i = 0; i < route.length - 1; i++) {
      for (let j = i + 2; j < route.length; j++) {
        ...
        if (newDist < currentDist) {
          ...
          improved = true;
          Logger.log(
            `[TSP]    Iteration ${iterations}: Improved by reversing segment [${i + 1}, ${j}]`,
          );
          break;
        }
      }
...
function calculateRouteMetrics(route, hqCoords) {
  ...
  return {
    ...
    averageSegmentDistance:
      Math.round((totalDistance / (route.length + 1)) * 100) / 100,
...
function sortClustersByDistanceFromHQ(clusters) {
  Logger.log(
    `[TSP] 🗺️ Sorting ${clusters.length} clusters by distance from HQ...`,
  );
  ...
  sorted.forEach((cluster, index) => {
    Logger.log(
      `[TSP]   ${index + 1}. ${cluster.id}: ${Math.round(cluster.distance_hq)}km from HQ (${cluster.nombre_livraisons} deliveries)`,
    );
  });
...
function TEST_compareOptimizationMethods() {
  ...
  // Get a sample route
  const routes = filterData(
    CONFIG.SHEETS.ROUTES,
    (row) => row.statut === CONFIG.ENUMS.STATUT_ROUTE.BROUILLON,
  );
  ...
  Logger.log(`\n🏆 RESULTS:`);
  const improvement_nn = (
    ((metrics1.totalDistance - metrics2.totalDistance) /
      metrics1.totalDistance) *
    100
  ).toFixed(1);
  const improvement_2opt = (
    ((metrics1.totalDistance - metrics3.totalDistance) /
      metrics1.totalDistance) *
    100
  ).toFixed(1);
  ...
  Logger.log(
    `   2-opt adds: ${(improvement_2opt - improvement_nn).toFixed(1)}% extra improvement`,
  );
