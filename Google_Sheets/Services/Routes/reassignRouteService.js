...
function reassignRoute(params) {
    Logger.log(`[RÉASSIGN] 🚀 Début réassignation route ${params.id_route} → bénévole ${params.id_benevole}`);
    ...
    if (!vehicule) throw new Error(`Aucun véhicule valide pour le bénévole ${params.id_benevole}`);
    ...
    const { assignees, surplus } = _fitLivraisons(
        livraisonsExistantes, capaciteKg, nombrePartMax, poidsParPart, poidsParPartHotel
    );

    Logger.log(`[RÉASSIGN] Capacité véhicule: ${capaciteKg}kg — assignées: ${assignees.length}, surplus: ${surplus.length}`);
    ...
    const capaciteOrigKg = _getCapaciteRouteOrigine(livraisonsExistantes, poidsParPart, poidsParPartHotel);
    const plusGrandVehicule = capaciteKg > capaciteOrigKg;

    if (plusGrandVehicule && hqCoords) {
        Logger.log(`[RÉASSIGN] Véhicule plus grand — recherche de stops supplémentaires proches`);
        const extras = _chercherStopsProches(
            params.id_route, livraisonsExistantes, route.date_debut,
            capaciteKg, nombrePartMax, poidsParPart, poidsParPartHotel, livraisonsFinales
        );
        if (extras.length > 0) {
            Logger.log(`[RÉASSIGN] ${extras.length} stop(s) supplémentaire(s) trouvé(s)`);
    ...
    if (surplus.length > 0) {
        ...
        Logger.log(`[RÉASSIGN] ${surplus.length} livraison(s) libérée(s) → Non Assignée`);
    }
    ...
    const poidsTotal = _calculerPoidsTotal(livraisonsOptimisees, poidsParPart, poidsParPartHotel);
    ...
    try {
        ...
        Logger.log(`[RÉASSIGN] 📄 Feuille imprimable régénérée pour ${params.id_route}`);
    } catch (err) {
    ...
    Logger.log(`[RÉASSIGN] ✅ Route ${params.id_route} réassignée à ${benevoleNom}`);
...
function splitRoute(params) {
    Logger.log(`[DIVISION] 🚀 Division route ${params.id_route} → ${(params.benevole_ids || []).length} bénévoles`);
    ...
    Logger.log(`[DIVISION] ${benevoles.length} bénévole(s) chargé(s), ${livraisons.length} livraison(s) à répartir`);

    const attribution = _repartirLivraisons(livraisons, benevoles, poidsParPart, poidsParPartHotel);
    ...
    if (attribution.nonAssignees.length > 0) {
        ...
        Logger.log(`[DIVISION] ${attribution.nonAssignees.length} livraison(s) non assignée(s) libérée(s)`);
    }
    ...
    for (const tranche of attribution.tranches) {
        ...
        const nouvelleRoute = _creerNouvelleRoute(
            tranche, route, hqCoords, poidsParPart, poidsParPartHotel
        );
        ...
        if (etaitPrete && apiWebUrl) {
            try {
                ...
                Logger.log(`[DIVISION] 📧 Email envoyé pour nouvelle route ${nouvelleRoute.id_route}`);
                nouvelleRoute.email_envoye = true;
            } catch (err) {
                Logger.log(`[DIVISION] ⚠️ Email non envoyé pour ${nouvelleRoute.id_route} : ${err.message}`);
    ...
    Logger.log(`[DIVISION] ✅ ${nouvellesRoutes.length} nouvelle(s) route(s) créée(s)`);
...
function _fitLivraisons(livraisons, capaciteKg, nombrePartMax, poidsParPart, poidsParPartHotel) {
    const assignees = [];
    const surplus = [];
    let poidsActuel = 0;
    let partsActuel = 0;
    ...
    for (const liv of livraisons) {
        ...
        if (poidsActuel + poids <= capaciteKg && partsActuel + parts <= nombrePartMax) {
            assignees.push(liv);
            poidsActuel += poids;
            partsActuel += parts;
        } else {
...
function _libererLivraisons(livraisons, routeId) {
    for (const liv of livraisons) {
        updateDeliveryStatus(liv.id_livraison, CONFIG.ENUMS.STATUT_LIVRAISON.NON_ASSIGNEE);

        const etapes = filterData(CONFIG.SHEETS.ETAPES_ROUTE,
            row => row.id_route === routeId && row.id_livraison === liv.id_livraison
        );
...
function _supprimerEtapesRoute(routeId) {
    ...
    Logger.log(`[RÉASSIGN] 🗑️ ${etapes.length} étape(s) supprimée(s) pour route ${routeId}`);
}

function _chercherStopsProches(routeId, livraisonsExistantes, dateLivraison, capaciteKg, nombrePartMax, poidsParPart, poidsParPartHotel, dejaAssignees) {
    ...
    const poidsDejaAssigne = _calculerPoidsTotal(dejaAssignees, poidsParPart, poidsParPartHotel);
    const partsDejaAssignees = dejaAssignees.reduce((s, l) => s + (parseInt(l.nombre_personnes) || 0), 0);

    const dateObj = dateLivraison instanceof Date ? dateLivraison : new Date(dateLivraison);
    ...
    for (const liv of livraisonsDisponibles) {
        ...
        if (poidsActuel + poids <= capaciteKg && partsActuel + parts <= nombrePartMax) {
            ...
            poidsActuel += poids;
            partsActuel += parts;
            updateDeliveryStatus(liv.id_livraison, CONFIG.ENUMS.STATUT_LIVRAISON.ASSIGNEE);
        }
    }
...
function _chargerBenevolesAvecVehicule(benevoleIds) {
    ...
    if (resp && resp.vehicles) {
        resp.vehicles.forEach(v => { vehiclesMap[v.id] = v; });
    }

    return benevoleIds.map(id => {
...
function _repartirLivraisons(livraisons, benevoles, poidsParPart, poidsParPartHotel) {
    ...
    for (const liv of livraisons) {
        ...
        for (const tranche of tranches) {
            ...
            if (poidsFutur <= tranche.benevole.capaciteKg && partsFutur <= tranche.benevole.nombrePartMax) {
...
function _creerNouvelleRoute(tranche, routeOriginale, hqCoords, poidsParPart, poidsParPartHotel) {
    try {
        let livraisonsOptimisees = tranche.livraisons;
        if (hqCoords && tranche.livraisons.length > 1) {
            livraisonsOptimisees = optimizeDeliveryOrder(tranche.livraisons, hqCoords);
        }
        ...
        const routeId = generateNextId(CONFIG.SHEETS.ROUTES, 'R', CONFIG.COLUMNS.ROUTES.ID_ROUTE);
        ...
        Logger.log(`[DIVISION] ✅ Nouvelle route ${routeId} créée pour ${tranche.benevole.nom}`);
        ...
        try {
            ...
            Logger.log(`[DIVISION] ⚠️ Feuille non générée pour ${routeId} : ${err.message}`);
        }
        ...
        Logger.log(`[DIVISION] ❌ Erreur création route pour ${tranche.benevole.nom} : ${err.message}`);
        return null;
    }
}