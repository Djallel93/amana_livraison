...
function generateRouteDoc(routeId, adminPhone) {
    ...
    _construireFeuille(ss, routeId, benevoleNom, dateRoute, stopsAvecDonnees, adminPhone);
...
function _enrichirStops(stops) {
    return stops.map(function (stop) {
        ...
        try {
            ...
            Logger.log(`[SHEET_ROUTE] ⚠️ Téléphone famille ${livraison.id_famille} : ${err.message}`);
        }
 ...
 * @param {string}  adminPhone
 */
function _construireFeuille(ss, routeId, benevoleNom, dateRoute, stops, adminPhone) {
    ...
    // ── Ligne 4 : En-têtes ────────────────────────────────────────────────────
    const headers = ['#', 'Famille', 'Adresse', 'Téléphone', 'Tél. bis', 'Personnes'];
    ...
    if (stops.length > 0) {
        ...
        sheet.getRange(currentRow, 6, rows.length, 1).setHorizontalAlignment('center');
    ...
    if (stops.length > 0) {
        const tableRange = sheet.getRange(dataStartRow - 1, 1, stops.length + 1, NB_COLS);
    ...
    Logger.log(`[SHEET_ROUTE] ✅ Feuille construite : ${stops.length} stop(s), admin=${adminPhone || 'aucun'}`);
}
