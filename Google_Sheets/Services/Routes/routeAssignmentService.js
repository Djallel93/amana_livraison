...
function assignerClustersAuxVehicules(clusters, benevoles, params) {
  ...
  if (hqCoords) {
    Logger.log(
      `[ROUTES] 🏢 QG: ${hqConfig.address} (${hqCoords.lat}, ${hqCoords.lng})`,
    );
  } else {
  ...
  Logger.log(
    `[ROUTES] 🚗 Début assignation incrémentale: ${benevoles.length} bénévoles, ${fileClusters.length} clusters`,
  );
  ...
  while (fileClusters.length > 0) {
    ...
    if (benevolesAvecRoute.size >= benevoles.length) {
      Logger.log(
        `[ROUTES] 🛑 Tous les bénévoles ont leur route — ${fileClusters.length} cluster(s) restant(s) pour la prochaine exécution`,
      );
      break;
    }
    ...
    if (++garde_fou > MAX_ITERATIONS) {
      Logger.log(
        `[ROUTES] ⚠️ Garde-fou atteint (${garde_fou} itérations) — arrêt`,
      );
      break;
    }
    ...
    Logger.log(
      `[ROUTES] 🔄 Cluster ${cluster.id}: ${cluster.nombre_livraisons} livraisons, ${cluster.nombre_parts} parts, ${Math.round(cluster.poids_total)}kg`,
    );
    ...
    if (candidat) {
      ...
      Logger.log(
        `[ROUTES] ✅ Cluster ${cluster.id} → ${candidat.nom} (${candidat.vehicule.type}, ${candidat.vehicule.capaciteKg}kg)`,
      );
    } else {
      // Aucun bénévole libre ne peut prendre ce cluster → tentative de split.
      const spliteur = chercherPlusGrandBenevoleLibre(
        benevoles,
        benevolesAvecRoute,
      );
      ...
      if (!spliteur) {
        Logger.log(
          `[ROUTES] ⚠️ Aucun bénévole disponible pour cluster ${cluster.id} — cluster ignoré pour cette exécution`,
        );
        continue;
      }

      Logger.log(
        `[ROUTES] ✂️ Split du cluster ${cluster.id} avec ${spliteur.nom} (${spliteur.vehicule.type})`,
      );
      ...
      Logger.log(
        `[ROUTES]   → Sous-cluster A: ${sousCluster.nombre_livraisons} livraisons, ${Math.round(sousCluster.poids_total)}kg`,
      );

      if (reste && reste.nombre_livraisons > 0) {
        fileClusters.unshift(reste);
        Logger.log(
          `[ROUTES]   → Reste: ${reste.nombre_livraisons} livraisons remises en file`,
        );
      }
    }
  }
  ...
  Logger.log(
    `[ROUTES] ✅ Assignation terminée: ${routes.length} routes, ${benevolesAvecRoute.size}/${benevoles.length} bénévoles utilisés`,
  );
  return routes;
}
 ...
 */
function chercherMeilleurBenevole(
  cluster,
  benevoles,
  benevolesAvecRoute,
  maxLivraisonsParRoute,
...
function splitCluster(
  ...
  const poids_moyen_kg =
    parseFloat((params && params.poids_moyen_kg) || poidsParPart) || 0;
  const poids_moyen_hotel_kg =
    parseFloat((params && params.poids_moyen_hotel_kg) || poids_moyen_kg) || 0;
  ...
  const partsB = livraisonsB.reduce(
  ...
  const centreB =
    livraisonsB.length > 0 ? calculerCentre(livraisonsB) : centreA;
