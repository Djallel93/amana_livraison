function createManualRoute(params) {
  ...
  Logger.log(
    `[ROUTE MANUELLE] Bénévole: ${params.id_benevole}, Livraisons: ${(params.livraison_ids || []).join(", ")}`,
  );
  ...
  if (vehicleResponse && vehicleResponse.vehicles) {
    vehicleResponse.vehicles.forEach((v) => {
      vehiclesMap[v.id] = v;
    });
  }
  ...
  for (const livraisonId of params.livraison_ids) {
    ...
    if (livraison.statut !== CONFIG.ENUMS.STATUT_LIVRAISON.NON_ASSIGNEE) {
      throw new Error(
        `Livraison ${livraisonId} n'est plus disponible (statut: ${livraison.statut})`,
      );
    }
  ...
  if (hqCoords && livraisons.length > 1) {
    Logger.log(
      `[ROUTE MANUELLE] 🎯 Optimisation TSP des ${livraisons.length} livraisons…`,
    );
    livraisonsOptimisees = optimizeDeliveryOrder(livraisons, hqCoords);
  } else {
    Logger.log(
      `[ROUTE MANUELLE] ⚠️ Optimisation ignorée (QG non configuré ou livraison unique)`,
    );
  }
  ...
  try {
    ...
    Logger.log(
      `[ROUTE MANUELLE] 📄 Feuille imprimable générée pour ${routeId}`,
    );
  } catch (docErr) {
  ...
  Logger.log(
    `[ROUTE MANUELLE] 🎉 Route ${routeId} créée — ${livraisonsOptimisees.length} livraisons, ${Math.round(distanceTotale)}km`,
  );
