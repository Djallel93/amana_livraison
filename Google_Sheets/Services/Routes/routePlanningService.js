...
function planRoutes(params) {
  ...
  try {
    ...
    Logger.log(
      `[ROUTES] 📦 ${livraisons.length} livraisons non assignées trouvées`,
    );
    ...
    if (outliers.length > 0) {
      Logger.log(
        `[ROUTES] ⚠️ ${outliers.length} livraisons isolées (> ${CONFIG.ROUTE_OPTIMIZATION.OUTLIER_DISTANCE_KM}km)`,
      );
      outliers.forEach((o) => {
...
function getBenevolesPourPlanification(params) {
  ...
  if (params.date_livraison) {
    try {
      ...
      if (permisVolunteers.length > 0) {
        ...
        Logger.log(
          `[ROUTES] 🚗 ${permisVolunteers.length} bénévole(s) permis ajouté(s) via véhicules temporaires`,
        );
      }
    } catch (err) {
      Logger.log(
        `[ROUTES] ⚠️ Erreur intégration véhicules temporaires: ${err.message}`,
      );
    }
  }
  ...
  if (params.selected_benevole_ids && params.selected_benevole_ids.length > 0) {
    ...
    Logger.log(
      `[ROUTES] 🎯 Filtre sélection manuelle: ${benevoles.length} bénévole(s) retenus`,
    );
  }
