...
const HEADERS_VEHICLE_TMP = ['id', 'id_type_vehicule', 'disponibilite_debut', 'disponibilite_fin', 'actif'];
...
function initVehicleTmpSheet() {
    ...
    if (!sheet) {
        ...
        sheet.getRange(1, 1, 1, HEADERS_VEHICLE_TMP.length).setValues([HEADERS_VEHICLE_TMP]);
...
function saveTmpVehicleFromForm(formData) {
    try {
        ...
        const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET_VEHICLE_TMP);
...
function getAvailableTmpVehicles(dateLivraison) {
    ...
    Logger.log(`[TMP_VEHICLE] ✅ ${enriched.length} véhicule(s) temporaire(s) disponible(s) pour ${dateLivraison}`);
    return enriched;
}
...
function getPermisVolunteers() {
    ...
    Logger.log(`[TMP_VEHICLE] ✅ ${permis.length} bénévole(s) permis sans véhicule`);
    return permis;
}
