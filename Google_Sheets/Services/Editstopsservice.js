...
function removeStop(params) {
  Logger.log(
    `[EDIT STOPS] 🗑️ Suppression stop — étape: ${params.id_etape}, livraison: ${params.id_livraison}, route: ${params.id_route}`,
  );
  ...
  const livraisons = stopsRestants
  ...
  const poidsTotal = _calculerPoidsStops(
  ...
  Logger.log(
    `[EDIT STOPS] ✅ Stop supprimé — ${livraisonsOptimisees.length} stop(s) restant(s)`,
  );
...
function addStop(params) {
  Logger.log(
    `[EDIT STOPS] ➕ Ajout livraison ${params.id_livraison} à route ${params.id_route}`,
  );
  ...
  if (!livraison)
    throw new Error(`Livraison ${params.id_livraison} introuvable`);

  if (livraison.statut !== CONFIG.ENUMS.STATUT_LIVRAISON.NON_ASSIGNEE) {
    throw new Error(
      `Livraison ${params.id_livraison} n'est plus disponible (statut: ${livraison.statut})`,
    );
  }
  ...
  if (!conditionnementPret) {
    ...
    Logger.log(
      `[EDIT STOPS] ⚠️ Colis non prêt — route rétrogradée à Brouillon`,
    );
  } else if (etaitPrete) {
    emailEnvoye = _envoyerEmailSiConfigured(params.id_route);
  }
  ...
  Logger.log(
    `[EDIT STOPS] ✅ Stop ajouté — ${livraisonsOptimisees.length} stop(s) au total, statut route: ${nouveauStatut}`,
  );
...
function getNearbyAvailableDeliveries(routeId) {
  Logger.log(
    `[EDIT STOPS] 🔍 Recherche livraisons disponibles pour route ${routeId}`,
  );
  ...
  Logger.log(
    `[EDIT STOPS] ✅ ${nearby.length} proches, ${fallback.length} autres disponibles`,
  );
...
function _reconstruireEtapes(routeId, livraisonsOptimisees) {
  const etapesExistantes = filterData(
    CONFIG.SHEETS.ETAPES_ROUTE,
  ...
  Logger.log(
    `[EDIT STOPS] 🗑️ ${etapesExistantes.length} étape(s) supprimée(s) pour reconstruction`,
  );
  createOptimizedEtapes(routeId, livraisonsOptimisees);
  Logger.log(
    `[EDIT STOPS] ✅ ${livraisonsOptimisees.length} étape(s) reconstruite(s) pour route ${routeId}`,
  );
}
...
function _regenererFeuille(routeId) {
  try {
    ...
    Logger.log(
      `[EDIT STOPS] 📄 Feuille imprimable régénérée pour route ${routeId}`,
    );
  } catch (err) {
    Logger.log(
      `[EDIT STOPS] ⚠️ Feuille non régénérée pour route ${routeId} : ${err.message}`,
    );
  }
}
...
function _envoyerEmailSiConfigured(routeId) {
  ...
  if (!apiWebUrl) {
    Logger.log(
      `[EDIT STOPS] ⚠️ API_LIVRAISON_URL non configurée — email non envoyé`,
    );
    return false;
  }
  try {
    ...
    Logger.log(
      `[EDIT STOPS] ⚠️ Email non envoyé pour route ${routeId} : ${err.message}`,
    );
    return false;
  }
}
