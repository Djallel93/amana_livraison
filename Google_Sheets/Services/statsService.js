...
function getStatsData(params) {
  Logger.log(
    "[STATS] 🚀 Calcul des statistiques pour " +
      params.date +
      " / " +
      params.occasion,
  );
...
function _statsStatuts(date, occasion) {
  ...
  const termines =
    (counts[CONFIG_ENUMS.STATUT_LIVRAISON.LIVREE] || 0) +
...
function _statsAjustement(date, occasion) {
  ...
  const routesAvecPoids = routes.filter(function (r) {
    return (
      parseFloat(r.poids_par_part) > 0 && parseFloat(r.poids_par_part_hotel) > 0
    );
  });
  ...
  if (routesAvecPoids.length > 0) {
    ...
    ratioHotel =
      parseFloat(derniere.poids_par_part_hotel) /
      parseFloat(derniere.poids_par_part);
  }
  ...
  if (totalParts > 0 && poidsDonne > 0) {
    ...
    if (ratioHotel !== null) {
      ...
      if (denominateur > 0) {
        ...
        poidsRecommandeHotel =
          Math.round(poidsRecommandeDom * ratioHotel * 10) / 10;
      }
    } else {
...
function sendStatsEmail(statsData, params) {
  ...
  if (emails.length === 0) {
    return {
      success: false,
      error: "Aucun destinataire valide dans STATS_EMAILS.",
    };
  }
  ...
  try {
    ...
    Logger.log(
      "[STATS] ✅ Email envoyé à " + emails.length + " destinataire(s)",
    );
...
function _buildStatsEmailHtml(statsData, params) {
  ...
  const statusRows = Object.entries(st.counts)
    .map(function (entry) {
      return (
        '<tr><td style="padding:6px 12px;border-bottom:1px solid #e2e8f0;">' +
        entry[0] +
  ...
  const recoDomStr =
    aj.poidsRecommandeDom !== null ? aj.poidsRecommandeDom + " kg/pers" : "N/A";
  const recoHotelStr =
    aj.poidsRecommandeHotel !== null
  ...
  return (
    ...
    '<div style="font-size:28px;font-weight:700;color:#4facfe;">' +
    g.totalLivraisons +
    "</div>" +
    ...
    '<div style="background:linear-gradient(135deg,#4facfe,#00f2fe);height:100%;width:' +
    st.pourcentageTermine +
    '%;border-radius:20px;"></div>' +
    "</div>" +
    '<p style="text-align:center;font-size:13px;color:#718096;margin:0 0 20px;">' +
    st.pourcentageTermine +
    "% complété</p>" +
    // Statuts
    ...
    '<tr><td style="padding:8px 12px;background:#f0f7ff;border-radius:8px 8px 0 0;font-size:13px;">⚖️ Poids donné (' +
    p.nbDons +
    " don(s))</td>" +
    '<td style="padding:8px 12px;background:#f0f7ff;border-radius:8px 8px 0 0;font-weight:700;text-align:right;">' +
    p.poidsDonne +
    " kg</td></tr>" +
    ...
    '<td style="padding:8px 12px;background:#f0f7ff;border-radius:0 0 8px 8px;font-weight:700;text-align:right;">' +
    aj.poidsProjecte +
    " kg</td></tr>" +
    "</table>" +
    // Ajustement
    ...
    aj.tauxCouverture +
    "%</td></tr>" +
    '<tr><td style="padding:8px 12px;background:#f0f7ff;font-size:13px;">' +
    surplusLabel +
    "</td>" +
    '<td style="padding:8px 12px;background:#f0f7ff;font-weight:700;text-align:right;color:' +
    surplusColor +
    ';">' +
    surplusSign +
    aj.surplusDeficit +
    ...
    (aj.nbLivraisonsNonAssignees > 0
      ? '<p style="font-size:11px;color:#a0aec0;margin:0 0 20px;">⚠️ ' +
        aj.nbLivraisonsNonAssignees +
        " livraison(s) non assignée(s) non incluses dans le calcul projeté.</p>"
      : "") +
    "</td></tr>" +
...
function sendScheduledStatsEmail() {
  ...
  const params = {
    date,
    occasion,
    poids_moyen_kg: poidsDom,
    poids_moyen_hotel_kg: poidsHot,
  };
  const stats = getStatsData(params);

  if (stats.global.totalLivraisons === 0) {
    Logger.log(
      "[STATS PLANIFIÉ] ⏭️ Aucune livraison trouvée, email non envoyé",
    );
    return;
  }
