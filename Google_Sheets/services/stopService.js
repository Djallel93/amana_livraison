  ...
  try {
    for (const routeId of routeIds) {
      try {
        ...
        // 3. Optimiser l'ordre des étapes
        const orderedStops = optimizeStopsOrder(livraisons);
        Logger.log(`[STOPS]   ✅ Ordre optimisé`);

        // 4. Créer les étapes
        const etapes = createStopsForRoute(routeId, orderedStops);
        Logger.log(`[STOPS]   ✅ ${etapes.length} étapes créées`);

        // 5. Générer le document Google
        const docUrl = generateRouteDocument(routeId, route, etapes);
        Logger.log(`[STOPS]   ✅ Document généré: ${docUrl}`);

        // 6. Envoyer l'email au bénévole
        const emailSent = sendRouteEmail(routeId, route, etapes, docUrl);
        if (emailSent) {
          Logger.log(`[STOPS]   ✅ Email envoyé au bénévole`);
        }

        // 7. Mettre à jour le statut de la route
...
/**
 * Crée les étapes pour une route
 * @param {string} routeId - ID de la route
 * @param {Array} orderedLivraisons - Livraisons ordonnées
 * @returns {Array} Étapes créées
 */
function createStopsForRoute(routeId, orderedLivraisons) {
  const etapes = [];

  for (let i = 0; i < orderedLivraisons.length; i++) {
    const livraison = orderedLivraisons[i];
    ...
    const etape = {
      ...
      ordre_passage: i + 1,
      statut: CONFIG.ENUMS.STATUT_ETAPE.EN_ATTENTE,
      heure_debut: null,
      heure_fin: null,
      commentaire: ''
    };
    ...
    // Valider
    const validation = validateEtape(etape);
    if (validation.hasErrors()) {
      Logger.log(`[STOPS] ⚠️ Étape invalide: ${validation.getErrorMessages().join(', ')}`);
      continue;
    }
    ...
    appendRow(CONFIG.SHEETS.ETAPES_ROUTE, rowData);
 ...
 * @param {Array} etapes - Étapes avec livraisons
 * @returns {string} URL du document
 */
function generateRouteDocument(routeId, route, etapes) {
  try {
    ...
    etapes.forEach((etape, index) => {
      const livraison = etape.livraison;
      tableData.push([
        String(index + 1),
 ...
 * @returns {boolean}
 */
function sendRouteEmail(routeId, route, etapes, docUrl) {
  try {
    ...
    // 2. Générer le token sécurisé
    const token = generateSecureToken();
    saveToken(routeId, token);

    // 3. Construire l'URL Google Maps pour toute la route
    const mapsUrl = buildGoogleMapsUrl(etapes, route.relivre);
 ...
 */
function buildGoogleMapsUrl(etapes, relivre) {
  const waypoints = [];
...
function buildRouteEmailHtml(data) {
  const { route, benevole, etapes, docUrl, mapsUrl, token } = data;

  // URL de base de l'API web (à définir dans les propriétés)
...
<body>
  ...
  <div class="content">
    ...
    <div class="info-box">
      ...
      <p>
        <strong>Nombre d'arrêts :</strong> ${etapes.length}<br>
