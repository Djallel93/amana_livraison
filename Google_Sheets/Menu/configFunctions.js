...
function saveApiKeysConfig(config) {
  try {
    ...
    if (config.famillesUrl)
      props.setProperty("API_FAMILLES_URL", config.famillesUrl);
    if (config.famillesKey)
      props.setProperty("API_FAMILLES_KEY", config.famillesKey);
    if (config.benevolesUrl)
      props.setProperty("API_BENEVOLES_URL", config.benevolesUrl);
    if (config.benevolesKey)
      props.setProperty("API_BENEVOLES_KEY", config.benevolesKey);
...
function getCurrentEmailConfig() {
  ...
  return {
    ...
    subjectRoute:
      props.getProperty("EMAIL_SUBJECT_ROUTE") || CONFIG.EMAIL.SUBJECT_ROUTE,
    subjectSkip:
      props.getProperty("EMAIL_SUBJECT_SKIP") ||
      CONFIG.EMAIL.SUBJECT_ADMIN_SKIP,
  };
}
...
function getCurrentOptimizationConfig() {
  ...
  return {
    distanceProximite:
      parseFloat(props.getProperty("OPT_DISTANCE_PROXIMITE")) ||
      CONFIG.ROUTE_OPTIMIZATION.DISTANCE_PROXIMITE_KM,
    distanceClusterMax:
      parseFloat(props.getProperty("OPT_DISTANCE_CLUSTER_MAX")) ||
      CONFIG.ROUTE_OPTIMIZATION.DISTANCE_CLUSTER_MAX_KM,
    distanceRegroupeEloignes:
      parseFloat(props.getProperty("OPT_DISTANCE_REGROUPE")) ||
      CONFIG.ROUTE_OPTIMIZATION.DISTANCE_REGROUPE_ELOIGNES_KM,
    distanceLivraisonIsolee:
      parseFloat(props.getProperty("OPT_DISTANCE_ISOLEE")) ||
      CONFIG.ROUTE_OPTIMIZATION.DISTANCE_LIVRAISON_ISOLEE_KM,
    capaciteBerline:
      parseFloat(props.getProperty("OPT_CAPACITE_BERLINE")) || 200,
...
function saveOptimizationConfig(config) {
  const props = PropertiesService.getScriptProperties();
  props.setProperty(
    "OPT_DISTANCE_PROXIMITE",
    config.distanceProximite.toString(),
  );
  props.setProperty(
    "OPT_DISTANCE_CLUSTER_MAX",
    config.distanceClusterMax.toString(),
  );
  props.setProperty(
    "OPT_DISTANCE_REGROUPE",
    config.distanceRegroupeEloignes.toString(),
  );
  props.setProperty(
    "OPT_DISTANCE_ISOLEE",
    config.distanceLivraisonIsolee.toString(),
  );
