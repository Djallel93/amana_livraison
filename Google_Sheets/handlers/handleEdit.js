...
function onEditConditionnement(e) {
    ...
    if (range.getColumn() !== CONFIG.COLUMNS.LIVRAISON.STATUT_CONDITIONNEMENT) return;
    if (range.getValue() !== CONFIG.ENUMS.STATUT_CONDITIONNEMENT.PRETE) return;

    const livraisonId = sheet.getRange(range.getRow(), CONFIG.COLUMNS.LIVRAISON.ID_LIVRAISON).getValue();
    if (!livraisonId) return;

    Logger.log(`[CONDITIONNEMENT] ✏️ Édition manuelle détectée — ${livraisonId} → Prête`);
